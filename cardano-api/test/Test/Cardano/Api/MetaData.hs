...
-- The reason this does not round trip is because a list of pairs aliases with
-- a map.
...
genTxMetadataValue =
    ...
    Gen.choice
      ...
      , TxMetaList <$> Gen.list (Range.linear 1 5) genFlatTxMetadataValue
      , TxMetaMap <$> genPairList (Range.linear 1 5)
      ]
...
genByteString = BS.pack <$> Gen.list (Range.linear 0 32) Gen.latin1

genText :: Gen Text
genText = Text.pack <$> Gen.list (Range.linear 0 32) Gen.alphaNum
